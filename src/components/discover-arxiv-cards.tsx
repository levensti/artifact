 ...
 */
function parseArxivSearchOutput(output: string): {
  header: string | null;
  papers: ParsedPaper[];
} {
    ...
    <div className="my-2 space-y-2">
      ...
      <div className="grid grid-cols-1 gap-2">
        {papers.map((p, i) => (
          <article
            key={`${p.url || p.title}-${i}`}
            className="rounded-lg border border-border/70 bg-card/60 px-3 py-2.5 transition-colors hover:border-border"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex shrink-0 flex-col items-end gap-1.5">
                {p.arxivId ? (
                  <AddToLibraryButton arxivId={p.arxivId} title={p.title} />
                ) : null}
                {p.url ? (
                  <a
                    href={p.url}
